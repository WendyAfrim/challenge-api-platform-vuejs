...
class RegistrationController extends AbstractController
{
    ...
    public function verifyUserEmail(Request $request): JsonResponse|RedirectResponse|Response
    {
        $id = $request->get('id');
            if (null === $id) {
            return $this->redirect('http://localhost:4000/login?'.http_build_query(['status' => 'error']));
        }
        ...
        try {
            ...
            return $this->redirect('http://localhost:4000/login?'.http_build_query(['status' => 'expired']));
        } catch (VerifyEmailExceptionInterface $exception) {
            return $this->redirect('http://localhost:4000/login?'.http_build_query(['status' => 'error']));
        }
        return $this->redirect('http://localhost:4000/login?'.http_build_query(['status' => 'validated']));
    }
